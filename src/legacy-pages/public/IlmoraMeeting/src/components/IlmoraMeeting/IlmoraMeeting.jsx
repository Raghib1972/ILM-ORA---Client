...
export default function IlmoraMeeting() {
  ...
  const loadMeeting = useCallback(async () => {
    ...
    // Texora meetings branch off entirely here. Everything below this
    // block is the original, untouched logic for regular meetings.
    if (isTexoraMeeting) {
      try {
        ...
        // Stable per-browser identity, persisted so a reload/reconnect
        // by the same person is recognized as the same participant
        // instead of counting as a brand-new join every time.
        ...
        // Use the real name Texora already sent in `context` on create,
        // no name prompt needed. Falls back to "Guest" only if Texora
        // never sent a name.
