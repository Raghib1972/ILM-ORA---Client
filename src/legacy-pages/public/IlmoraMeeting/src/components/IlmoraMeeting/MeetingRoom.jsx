...
export function MeetingRoom({
  ...
  const localScreenRef = useRef(null); // tracks the active screen-share track so refresh can republish it
  const micOnRef = useRef(true); // mirrors micOn state, read inside setInterval closures so refresh always sees the LATEST value, not a stale one captured at mount
  const camOnRef = useRef(true); // mirrors camOn state, same reason
  ...
  // Keep the refs in sync with state on every change, so the setInterval
  // closure in doTokenRefresh always reads the CURRENT mic/cam status
  // instead of whatever it was at the moment of mount.
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    statusPollRef.current = setInterval(async () => {
      try {
        // Texora meetings poll their own validate endpoint instead of the
        // regular one, since they live in a separate table.
        if (isTexora) {
  ...
  const toggleScreen = useCallback(async () => {
    ...
    try {
      ...
      if (!pub) return;
      localScreenRef.current = pub.track || null; // remember it for refresh republish
      setScreenOn(true);
      setMediaError(null);
      rebuild();
    ...
    <div style={S.root} className="im-root" data-theme={theme}>
      ...
      <div style={S.topBar} className="im-topbar">
        <div style={S.topLeft} className="im-topleft">
          <img src={texoraLogo.src} alt="Texora AI" style={S.logoImg} />
