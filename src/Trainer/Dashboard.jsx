...
import { useNavigate, useLocation, useSearchParams } from "@/lib/routerCompat";
import {
  Users,
  BookOpen,
...
//
// COLOR TOKENS: every icon/badge on this page is now the same neutral
// black/grey (t.text on t.iconBg) that the Student dashboard uses for
// every one of its cards (My Classroom, Video/Document/Assignment/Quiz
// Progress, the "● LIVE" badge, etc.) — no page-local accent hex/rgba
// anywhere. ACCENT_PURPLE (the shared Claude accent) is kept only for
// the single small eyebrow dot next to "Trainer Portal", which mirrors
// the identical dot Student's "STUDENT PORTAL" eyebrow uses.
import {
  T,
  FONT_FAMILY,
  FONT_WEIGHT,
...
const Dashboard = () => {
  ...
  // Dark-mode detection now goes through the shared design-system hook —
  // same "dark" class / data-theme observer every other dashboard
  // (Student/Admin) uses, instead of a page-local copy.
  ...
  // Panel icon color — neutral black/grey (t.text on t.iconBg), matching
  // the Student dashboard's monochrome icon treatment exactly (My
  // Classroom / Video Progress / Documents Progress etc. all render as
  // plain black icons in a soft grey badge — no per-card accent color).
  // StatCard's own icon (top 8 cards) already renders neutral for the
  // same reason; these page-local panels now follow the same rule.
  const iconColor = t.text;
  const iconBg = t.iconBg;
  const iconBorder = t.iconBorder;
    ...
    <PageContainer mode={isDark ? "dark" : "light"} pageBg={t.pageBg} textColor={t.text}>
      ...
      <Hero borderHero={t.borderHero}>
        ...
        <div className="hero-badges">
          {!loading && (
            ...
            >
              ...
              {doubtsLoading ? (
                ...
                <span style={{ display: "flex", alignItems: "center", gap: 5, color: t.text, fontWeight: 700 }}>
                  <span style={{ width: 6, height: 6, borderRadius: "50%", background: t.text, display: "inline-block" }} />
                  {counts.doubts} unanswered
                </span>
              ) : (
                <span style={{ color: t.textSub }}>No pending doubts ✓</span>
              )}
            </div>
          )}
          ...
          {/* LIVE badge — neutral grey/black pill, same as the Student
              dashboard's "● LIVE" badge (no purple/orange accent). */}
          <div
            className="livebadge"
            style={{
              display: "flex", alignItems: "center", gap: 7,
              background: t.actBg, border: `1px solid ${t.actBorder}`,
              borderRadius: RADIUS.pill, padding: "8px 18px", color: t.text,
      ...
      <div className="courses-grid" style={{ marginBottom: 14 }}>
        <RecentPanel
          ...
          t={t}
          renderItem={(v, i) => (
            <RecentRow key={i} color={iconColor} Icon={Video} title={v.title || v.fileName || "Untitled"} sub={fmt(v.uploadedAt || v.createdAt)} t={t} />
          )}
        />
        <RecentPanel
          title="Recent Files"
          ...
          t={t}
          renderItem={(f, i) => (
            <RecentRow key={i} color={iconColor} Icon={FileText} title={f.title || f.fileName || f.originalFileName || "Untitled"} sub={fmt(f.uploadedAt || f.createdAt)} t={t} />
          )}
        />
        <RecentPanel
      ...
      <div className="dash-row-grid" style={{ marginBottom: 14 }}>
        ...
        <div style={card} className="dfade">
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              <div style={{ width: 34, height: 34, borderRadius: RADIUS.chip, display: "flex", alignItems: "center", justifyContent: "center", background: iconBg, border: `1px solid ${iconBorder}` }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "28px 0", gap: 12 }}>
            ...
            <button
              ...
              style={{ padding: "6px 18px", borderRadius: RADIUS.button, border: `1px solid ${t.actBorder}`, background: t.actBg, color: t.text, fontSize: 11, fontWeight: 600, cursor: "pointer", fontFamily: FONT_FAMILY }}
            >
              View Reports →
            </button>
          </div>
