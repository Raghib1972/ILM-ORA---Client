...
import { FONT_FAMILY, FONT_SIZE } from "../tokens/typography";
...
const StatCard = ({ stat, index, loading, mode }) => {
  ...
  // Every StatCard uses ONE plain black/neutral treatment — icon, icon-badge
  // background, and the base progress line are all black (dark gray in dark
  // mode), matching the sidebar's monochrome look. No per-card accent color.
  // `stat.colorKey` is still accepted on the prop for backward compatibility
  // but no longer changes the rendered color.
  const c =
    resolvedMode === "dark"
      ? { solid: "#ffffff", soft: "rgba(255,255,255,0.08)" }
      : { solid: "#1F1E1B", soft: "rgba(31,30,27,0.06)" };

  const cardBg = resolvedMode === "dark" ? "#111111" : "#ffffff";
  const border = resolvedMode === "dark" ? "rgba(255,255,255,0.08)" : "#DEDAD2";
  const valueColor = resolvedMode === "dark" ? "#ffffff" : "#1F1E1B";
  const labelColor = resolvedMode === "dark" ? "rgba(255,255,255,0.55)" : "#83827A";
  const changeColor = resolvedMode === "dark" ? "rgba(255,255,255,0.42)" : "#9A968C";
  const shadow = hov
    ? resolvedMode === "dark" ? "0 10px 24px rgba(0,0,0,0.4)" : "0 8px 20px rgba(31,30,27,0.08)"
    : resolvedMode === "dark" ? "0 2px 10px rgba(0,0,0,0.3)" : "0 1px 6px rgba(31,30,27,0.05)";
  ...
  return (
    ...
    >
      ...
      <div>
        {loading ? (
          <div style={{ width: 56, height: 26, borderRadius: 6, background: resolvedMode === "dark" ? "rgba(255,255,255,0.1)" : "#F0EEE8" }} />
        ) : (
          // Fixed px size from the shared token (never vw/rem-scaled), so this
          // number can't render oversized regardless of value/content —
          // fixes the "0 renders huge" bug.
          <p style={{ fontSize: FONT_SIZE.cardValue, maxHeight: FONT_SIZE.cardValue * 1.1, fontWeight: 800, lineHeight: 1, fontFamily: FONT_FAMILY, color: valueColor, margin: 0, overflow: "hidden" }}>
            {stat.isPercent ? (stat.numericValue !== null ? `${stat.numericValue}%` : "—") : String(count)}
          </p>
        )}
        <p style={{ fontSize: FONT_SIZE.cardLabel, marginTop: 6, fontWeight: 600, color: labelColor, fontFamily: FONT_FAMILY, margin: "6px 0 0" }}>
          {stat.label}
        </p>
      </div>

      {stat.change && (
        <p style={{ fontSize: FONT_SIZE.bodySmall, fontWeight: 500, color: changeColor, fontFamily: FONT_FAMILY, margin: 0 }}>
          {stat.change}
        </p>
      )}
      ...
      <div
        style={{
          ...
          bottom: 0,
          height: 3,
          background: resolvedMode === "dark" ? "rgba(255,255,255,0.06)" : "#F0EEE8",
        }}
      >
        <div
          style={{
