...
import { T, StatCard, PageContainer, CLAUDE_ACCENT } from "@/design-system";
...
export default function StudentCourseView() {
  ...
  const load = async () => {
    try {
      const [courseRes, contentRes, planRes] = await Promise.all([
        ...
        axios
          .get(`${API}/courses/student/plan-summary`, { headers: authHeader() })
          .catch(() => ({ data: null })),
      ]);
  ...
  const playVideo = async (c, index) => {
    ...
    try {
      ...
      const presignedUrl = await videoService.getCourseVideoPlaybackUrl(fileName);
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ display: "flex", flexWrap: "wrap", gap: 4, marginBottom: 6 }}>
                        ...
                        {!unlocked && isPlanLocked(index) && (
                          <span style={{ fontSize: 9, fontWeight: 600, padding: "2px 7px", borderRadius: 20, background: t.statusProgressBg, color: t.statusProgressText, display: "inline-flex", alignItems: "center", gap: 3, fontFamily: "'Poppins',sans-serif" }}>
