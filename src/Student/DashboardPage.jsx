...
const getProgressColor = (pct) => {
  ...
  if (pct >= 60) return "#1F1E1B";
  if (pct >= 30) return "#1F1E1B";
  return "#94a3b8";
};
...
/* ═══════════════════════════════════════════════
   MINI PROGRESS CARD
═══════════════════════════════════════════════ */
const MiniProgressCard = ({ icon: Icon, label, color: _colorProp, done, total, pct, navigateTo, navigateLabel, t }) => {
  const navigate = useNavigate();
  // Monochrome pass: every progress card (Video/Documents/Assignment/Quiz)
  // now renders in one black/neutral tone instead of a different accent
  // color per card. The `color` prop is still accepted for backward
      ...
      {total === 0 ? (
        ...
        <>
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            ...
            {total - done > 0 && (
              <span style={{ fontSize: 11, fontWeight: FONT_WEIGHT.semibold, padding: "4px 10px", borderRadius: 8, background: "rgba(31,30,27,0.06)", color: t.text, fontFamily: FONT_FAMILY }}>{total - done} left</span>
            )}
          </div>
        </>
      )}
  ...
  return (
    <div style={{ position: "relative" }} ref={dropdownRef}>
      <button onClick={() => setOpen((p) => !p)} style={{ position: "relative", display: "flex", alignItems: "center", justifyContent: "center", width: 40, height: 40, borderRadius: 10, background: t.actBg, border: `1px solid ${t.actBorder}`, color: t.text, cursor: "pointer", transition: "all 0.2s" }}>
        ...
        {unreadCount > 0 && (
          <span style={{ position: "absolute", top: -2, right: -2, minWidth: 18, height: 18, borderRadius: 999, background: t.text, color: t.pageBg, fontSize: FONT_SIZE.eyebrow, fontWeight: FONT_WEIGHT.bold, display: "flex", alignItems: "center", justifyContent: "center", padding: "0 4px", fontFamily: FONT_FAMILY }}>
        ...
        <div style={{ position: "absolute", right: 0, top: "calc(100% + 8px)", width: 340, borderRadius: 16, border: `1px solid ${t.border}`, background: t.cardBg, boxShadow: t.shadowHov, zIndex: 9999, overflow: "hidden", display: "flex", flexDirection: "column", maxHeight: 480 }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 16px", borderBottom: `1px solid ${t.border}`, flexShrink: 0 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <Bell size={14} color={t.text} />
              <span style={{ fontSize: 13, fontWeight: FONT_WEIGHT.bold, color: t.text, fontFamily: FONT_FAMILY }}>Notifications</span>
              {unreadCount > 0 && <span style={{ padding: "2px 8px", fontSize: 10, fontWeight: FONT_WEIGHT.bold, borderRadius: 999, background: "rgba(31,30,27,0.06)", color: t.text, fontFamily: FONT_FAMILY }}>{unreadCount} new</span>}
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
              {unreadCount > 0 && (
                <button onClick={handleMarkAllRead} style={{ display: "flex", alignItems: "center", gap: 4, padding: "4px 8px", borderRadius: 8, fontSize: 11, color: t.text, cursor: "pointer", background: "transparent", border: "none", fontFamily: FONT_FAMILY, fontWeight: FONT_WEIGHT.semibold }}>
          ...
          <div style={{ overflowY: "auto", flex: 1 }}>
            ...
            {notifications.map((item) => {
              ...
              return (
                <div key={item.id} onClick={(e) => !item.read && handleMarkRead(item.id, e)} style={{ display: "flex", alignItems: "flex-start", gap: 10, padding: "10px 16px", borderBottom: `1px solid ${t.border}`, background: !item.read ? "rgba(31,30,27,0.04)" : "transparent", cursor: "pointer", transition: "background 0.15s" }}>
                  <div style={{ width: 36, height: 36, borderRadius: 10, flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center", background: "rgba(31,30,27,0.06)", border: "1px solid rgba(31,30,27,0.15)", fontSize: 14 }}>{icon}</div>
                  ...
                  {!item.read && <div style={{ width: 7, height: 7, borderRadius: "50%", background: t.text, flexShrink: 0, marginTop: 4 }} />}
                </div>
              );
            })}
          </div>
          <div style={{ flexShrink: 0, borderTop: `1px solid ${t.border}` }}>
            <button style={{ width: "100%", padding: "10px", fontSize: 11, fontWeight: FONT_WEIGHT.semibold, color: t.text, background: "transparent", border: "none", cursor: "pointer", fontFamily: FONT_FAMILY }}>View all notifications →</button>
          </div>
        </div>
      )}
    </div>
  ...
  if (!expanded) return (
    <div style={{ ...cardStyle, padding: 22, display: "flex", flexDirection: "column" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <div style={{ width: 34, height: 34, borderRadius: 10, display: "flex", alignItems: "center", justifyContent: "center", background: t.iconBg, border: `1px solid ${t.iconBorder}` }}>
            <MessageCircle size={15} color={t.text} />
          </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 6, marginBottom: 12, maxHeight: 140, overflowY: "auto" }}>
          {chat.slice(-3).map((msg, idx) => (
            <div key={idx} style={{ display: "flex", justifyContent: msg.sender === "student" ? "flex-end" : "flex-start" }}>
              <div style={{ maxWidth: "80%", padding: "8px 12px", borderRadius: 12, fontSize: 11, background: msg.sender === "student" ? t.text : t.recentItemBg, color: msg.sender === "student" ? t.pageBg : t.text, fontFamily: FONT_FAMILY }}>{msg.text}</div>
            </div>
          ))}
        </div>
      )}
      {batchId && trainerEmail && (
        <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
          ...
          <button onClick={handleSend} disabled={!message.trim() || sending} style={{ width: 34, height: 34, borderRadius: 10, background: t.text, border: "none", cursor: "pointer", color: t.pageBg, display: "flex", alignItems: "center", justifyContent: "center", opacity: (!message.trim() || sending) ? 0.4 : 1 }}>
            <Send size={13} />
          </button>
        </div>
      )}
      <div style={{ marginTop: "auto", paddingTop: 12, borderTop: `1px solid ${t.border}` }}>
        <button onClick={() => setExpanded(true)} style={{ width: "100%", display: "flex", alignItems: "center", justifyContent: "center", gap: 8, padding: "9px", borderRadius: 10, border: "1px solid rgba(31,30,27,0.2)", background: "rgba(31,30,27,0.05)", color: t.text, fontSize: 11, fontWeight: FONT_WEIGHT.semibold, cursor: "pointer", fontFamily: FONT_FAMILY }}>
  ...
  return (
    <div style={{ ...cardStyle, display: "flex", flexDirection: "column", height: 380 }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 16px", borderBottom: `1px solid ${t.border}` }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <div style={{ width: 28, height: 28, borderRadius: 999, background: "rgba(31,30,27,0.06)", display: "flex", alignItems: "center", justifyContent: "center" }}>
            <GraduationCap size={13} color={t.text} />
          </div>
          <div>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "12px 16px", display: "flex", flexDirection: "column", gap: 8 }}>
        {chat.length === 0 ? (
          ...
          <div key={idx} style={{ display: "flex", justifyContent: msg.sender === "student" ? "flex-end" : "flex-start", alignItems: "flex-end", gap: 6 }}>
            {msg.sender === "teacher" && <div style={{ width: 24, height: 24, borderRadius: 999, flexShrink: 0, background: "rgba(31,30,27,0.06)", display: "flex", alignItems: "center", justifyContent: "center" }}><GraduationCap size={11} color={t.text} /></div>}
            <div style={{ maxWidth: "75%", padding: "8px 12px", borderRadius: 12, fontSize: 11, background: msg.sender === "student" ? t.text : t.recentItemBg, color: msg.sender === "student" ? t.pageBg : t.text, fontFamily: FONT_FAMILY }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "10px 12px", borderTop: `1px solid ${t.border}` }}>
        ...
        <button onClick={handleSend} disabled={!message.trim() || sending || !batchId} style={{ width: 34, height: 34, borderRadius: 10, background: t.text, border: "none", cursor: "pointer", color: t.pageBg, display: "flex", alignItems: "center", justifyContent: "center", opacity: (!message.trim() || sending || !batchId) ? 0.4 : 1 }}>
          <Send size={13} />
        </button>
      </div>
    </div>
            ...
            >
              ...
              <div style={{ padding: "18px 20px 20px", flex: 1, display: "flex", flexDirection: "column" }}>
                ...
                <div style={{ marginTop: "auto" }}>
                  ...
                  <button
                    onClick={() => navigate(`/ilm-demo?section=${encodeURIComponent(`/student/course/${course.id}`)}`)}
                    style={{
                      width: "100%", padding: "10px", borderRadius: 12,
                      background: pct >= 100 ? status.bg : t.text,
                      border: pct >= 100 ? `1px solid ${color}40` : "none",
                      color: pct >= 100 ? color : t.pageBg,
  ...
  const ResourceBlock = ({ icon: Icon, label, color, done, total, pct, emptyMsg, navigateTo, navigateLabel }) => (
    <div style={{ background: t.cardBg, border: `1px solid ${t.border}`, borderRadius: 20, boxShadow: t.shadow, overflow: "hidden" }}>
      <div style={{ padding: "16px 22px", borderBottom: `1px solid ${t.border}`, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        {navigateTo && (
          <button onClick={() => navigate(navigateTo)} style={{ fontSize: 11, fontWeight: FONT_WEIGHT.semibold, color: t.text, background: "transparent", border: "none", cursor: "pointer", fontFamily: FONT_FAMILY }}>
            {navigateLabel} →
          </button>
        )}
      </div>
      {total === 0 ? (
        ...
        <div style={{ padding: "20px 22px" }}>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
            ...
            {total - done > 0 && <span style={{ fontSize: 11, fontWeight: FONT_WEIGHT.semibold, padding: "5px 12px", borderRadius: 8, background: "rgba(31,30,27,0.06)", color: t.text, fontFamily: FONT_FAMILY }}>{total - done} remaining</span>}
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <div className="progress-summary-grid">
        {[
          { label: "Total Courses", value: totalCourses, icon: BookOpen, color: t.text },
          { label: "Completed", value: completedCourses, icon: Trophy, color: "#34d399" },
          { label: "In Progress", value: inProgressCourses, icon: PlayCircle, color: t.text },
      ...
      {courses.length === 0 ? (
        ...
        <div style={{ background: t.cardBg, border: `1px solid ${t.border}`, borderRadius: 20, boxShadow: t.shadow, overflow: "hidden" }}>
          <div style={{ padding: "16px 22px", borderBottom: `1px solid ${t.border}`, display: "flex", alignItems: "center", gap: 10 }}>
            <BarChart2 size={16} color={t.text} />
      ...
      <ResourceBlock icon={ClipboardList} label="Assignment Progress" color={t.text} done={assignmentProgress.completedCount} total={assignmentProgress.totalAssignments} pct={assignmentProgress.percentage} emptyMsg="No assignments available yet" navigateTo="/student/assignments" navigateLabel="Go to Assignments" />
      <ResourceBlock icon={Brain} label="Quiz Progress" color={t.text} done={quizProgress.completedCount} total={quizProgress.totalQuizzes} pct={quizProgress.percentage} emptyMsg="No quizzes available yet" navigateTo="/student/assessments" navigateLabel="Go to Quizzes" />
    </div>
  );
};
      ...
      <div className="dash-row-grid" style={{ marginBottom: 14 }}>
        ...
        <div style={card}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              <div style={{ width: 34, height: 34, borderRadius: 10, display: "flex", alignItems: "center", justifyContent: "center", background: t.iconBg, border: `1px solid ${t.iconBorder}` }}><User size={15} color={t.text} /></div>
          ...
          {classLoading ? (
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {[
                { label: "Batch Name", value: classroom.batchName, color: t.text, Icon: GraduationCap },
                { label: "Trainer", value: classroom.trainerName, color: t.text, Icon: User },
                { label: "Trainer Email", value: classroom.trainerEmail, color: t.text, Icon: MessageCircle },
              ].map((row) => (
            ...
            <div className="hero-flex">
              ...
              <div>
                <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 8 }}>
                  <div style={{ width: 6, height: 6, borderRadius: "50%", background: CLAUDE_ACCENT }} className="d1" />
                ...
                <DashboardGreeting t={t} titleColor={t.text} />
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginTop: 14 }}>
                  {["overview", "courses", "progress"].map((tab) => (
                    <button key={tab} onClick={() => setActiveTab(tab)} style={{
                      ...
                      border: `1px solid ${activeTab === tab ? "rgba(31,30,27,0.45)" : t.borderHov}`,
                      background: activeTab === tab ? "rgba(31,30,27,0.08)" : t.actBg,
                      color: activeTab === tab ? t.text : t.textSub,
              ...
              <div className="hero-badges">
                {!statsLoading && (
                  <div style={{
                    ...
                    {pendingAssessments > 0
                      ? <span style={{ display: "flex", alignItems: "center", gap: 5, color: t.text, fontWeight: FONT_WEIGHT.bold }}><span style={{ width: 6, height: 6, borderRadius: "50%", background: t.text, display: "inline-block" }} />{pendingAssessments} pending</span>
                      : <span style={{ color: t.text }}>All clear ✓</span>
                    }
                  </div>
                )}
                ...
                <div className="livebadge" style={{ display: "flex", alignItems: "center", gap: 7, background: "rgba(31,30,27,0.06)", border: "1px solid rgba(31,30,27,0.2)", borderRadius: 999, padding: "8px 18px", color: t.text, fontSize: 11, fontWeight: FONT_WEIGHT.bold, letterSpacing: LETTER_SPACING.eyebrowWide, fontFamily: FONT_FAMILY }}>
                  <span style={{ width: 6, height: 6, borderRadius: "50%", background: t.text, display: "inline-block" }} />LIVE
                </div>
              </div>
            </div>
          </div>
