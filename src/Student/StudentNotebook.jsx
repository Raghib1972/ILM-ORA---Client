...
// ── Global Design System — the SAME single source of truth used by the
// Dashboard, Admin and Trainer pages (T, fonts, weights, spacing).
// This file must never invent its own colors: every color below is read
// from `T.light` / `T.dark` at render time and pushed into CSS custom
// properties, so this component always matches the rest of the LMS and
// stays in sync automatically if the design system changes.
import {
  T,
  CLAUDE_ACCENT,
  FONT_FAMILY,
...
/* ─── CSS Injection ───
   NOTE: this stylesheet is intentionally 100% free of hex/rgba color
   literals. Every color is a var(--token) whose *value* is supplied at
   runtime (see `themeVars()` below) straight from the design system's
   T.light / T.dark objects — exactly the same values the Dashboard uses.
*/
const INJECT_ID = "snb-lm-v5";
const GLOBAL_CSS = `
  ...
  @keyframes snblm-glow { 0%,100%{box-shadow:0 0 0 0 transparent} 50%{box-shadow:0 0 0 4px var(--accent-bg3)} }
  ...
  .snblm-scroll::-webkit-scrollbar-thumb { background:var(--scrollbar-thumb); border-radius:3px; }
  .snblm-scroll::-webkit-scrollbar-thumb:hover { background:var(--scrollbar-thumb-hov); }

   /* ── SHELL ── */
  .snblm-root {
    position:relative;
    height:100%; min-height:100vh;
    background:var(--bg); font-family:var(--font-family);
  ...
  .snblm-btn {
    ...
    padding:8px 16px; background:var(--accent); color:var(--accent-contrast);
  ...
  .snblm-search-results {
    ...
    box-shadow:var(--shadow-hov); animation:snblm-pop .15s ease;
  }
  .snblm-search-item {
  ...
  .snblm-sources-search input {
    border:none; outline:none; background:transparent;
    font-family:var(--font-family); font-size:11px; color:var(--text); flex:1;
  }
  ...
  .snblm-msg-save-btn {
    ...
    font-family:var(--font-family); font-size:11px; font-weight:600;
    color:var(--text2); cursor:pointer;
  ...
  .snblm-msg.user .snblm-msg-avatar { background:var(--accent); color:var(--accent-contrast); }
  ...
  .snblm-msg.user .snblm-msg-bubble { background:var(--accent); color:var(--accent-contrast); border-bottom-right-radius:4px; }
  ...
  .snblm-msg-bubble code { background:var(--code-bg); padding:1px 5px; border-radius:4px; font-size:11px; font-family:monospace; }
  .snblm-msg.user .snblm-msg-bubble code { background:var(--code-bg-inverse); }
  .snblm-msg-bubble pre { background:var(--code-bg); padding:9px 11px; border-radius:7px; overflow-x:auto; margin:7px 0; }
  ...
  .snblm-studio-btn:hover { background:var(--studio-hover); border-color:var(--accent-bdr); transform:translateY(-2px); box-shadow:var(--shadow-hov); }
  ...
  .snblm-note-title-inp {
    ...
    font-family:var(--font-family); font-size:18px; font-weight:800;
  ...
  .snblm-tbtn .tip {
    ...
    font-family:var(--font-family); z-index:60;
  }
  ...
  .snblm-ta {
    flex:1; resize:none; border:none; outline:none;
    font-family:var(--font-family); font-size:14px; line-height:2;
  ...
  .snblm-status-dot.saving { animation:snblm-blink .9s infinite; background:var(--warning); }
...
/* Original modals (no .snblm-modal-scroll body) keep their padded layout */
.snblm-modal:not(:has(> .snblm-modal-scroll)) {
  display:block; padding:24px; overflow-y:auto;
}
.snblm-modal:not(:has(> .snblm-modal-scroll)) .snblm-modal-actions {
  padding:0; margin-top:16px; border-top:none; background:transparent;
}
  .snblm-modal-title {
...
/* ── Runtime theme → CSS custom properties ──
   The ONLY place color values are decided. Everything comes from `t`
   (T.light or T.dark, exactly what Dashboard.jsx uses) plus FONT_FAMILY.
   No hex/rgba is invented here — where a token doesn't exist 1:1 on `t`
   we reuse the closest existing token rather than hardcoding a new color.
*/
const themeVars = (t, isDark) => ({
  "--font-family": FONT_FAMILY,
  "--bg": t.pageBg,
  "--surface": t.cardBg,
  "--surface2": t.recentItemBg,
  "--surface3": t.iconBg,
  "--border": t.border,
  "--border2": t.borderHov,
  "--text": t.text,
  ...
  "--text3": t.textMuted,
  "--text4": t.textLabel,
  // Monochrome accent — same convention the Dashboard uses (t.text as the
  // primary interactive color, t.pageBg as its contrast/foreground).
  "--accent": t.text,
  "--accent2": t.text,
  "--accent-contrast": t.pageBg,
  "--accent-bg": t.iconBg,
  "--accent-bg2": t.pillBg,
  "--accent-bg3": t.recentItemBg,
  "--accent-bdr": t.iconBorder,
  "--accent-sh": isDark ? "rgba(0,0,0,.35)" : "rgba(15,23,42,.10)",
  "--live": CLAUDE_ACCENT || t.liveColor,
  "--green": t.statusCompletedText,
  "--green-bg": t.statusCompletedBg,
  "--red": t.overdueText,
  "--red-bg": t.overdueBg,
  "--warning": t.newBadgeText,
  "--overlay": isDark ? "rgba(0,0,0,.7)" : "rgba(15,23,42,.45)",
  "--modal": t.cardBg,
  "--chat-user": t.text,
  "--chat-ai": t.recentItemBg,
  "--chat-ai-txt": t.text,
  "--src-hover": t.recentItemBgHov,
  "--studio-btn": t.cardBg,
  "--studio-hover": t.cardBgHov,
  "--toast-bg": t.text,
  "--toast-txt": t.pageBg,
  "--nb-shadow": t.shadow,
  "--nb-shadow-h": t.shadowHov,
  "--shadow": t.shadow,
  "--shadow-hov": t.shadowHov,
  "--scrollbar-thumb": t.border,
  "--scrollbar-thumb-hov": t.borderHov,
  "--code-bg": t.iconBg,
  "--code-bg-inverse": isDark ? "rgba(0,0,0,.25)" : "rgba(255,255,255,.25)",
});

/* ── Constants ──
   Notebook/section colors and tag colors are the ONE place a person is
   meant to pick a distinct color (to tell notebooks apart) — same idea as
   Dashboard's per-resource colors (video/docs/assignment/quiz). The exact
   hex values below are the same ones already used across the LMS
   (see Dashboard.jsx: #34d399, #22d3ee, #0ea5e9, #fb923c, #a78bfa, #f87171,
   #fbbf24) — nothing new is invented here.
*/
const NB_COLORS = [
  "#0f172a","#0ea5e9","#22d3ee","#34d399",
  "#fb923c","#a78bfa","#f87171","#fbbf24",
];
const SEC_COLORS = NB_COLORS;
const TAG_OPTIONS = [
  { label:"Important", icon:"⭐", color:"#fbbf24", bg:"rgba(251,191,36,.14)" },
  { label:"Question",  icon:"❓", color:"#0ea5e9", bg:"rgba(14,165,233,.12)" },
  { label:"To Do",     icon:"✅", color:"#34d399", bg:"rgba(52,211,153,.12)"  },
  { label:"Idea",      icon:"💡", color:"#a78bfa", bg:"rgba(167,139,250,.12)" },
  { label:"Remember",  icon:"🔖", color:"#f87171", bg:"rgba(248,113,113,.12)"  },
  { label:"Definition",icon:"📌", color:"#22d3ee", bg:"rgba(34,211,238,.12)"  },
];
...
const SRC_COLORS = { PDF:"#fb923c", WEBSITE:"#0ea5e9", YOUTUBE:"#f87171", TEXT:"#a78bfa", AUDIO:"#34d399", IMAGE:"#fbbf24", DRIVE:"#22d3ee" };
...
const StudentNotebook = ({ isDark: isDarkProp }) => {
  ...
  // Design-system theme object — identical to what Dashboard.jsx uses.
  const t = isDark ? T.dark : T.light;
  const cssVars = themeVars(t, isDark);
  ...
  useEffect(()=>{ if(!activeNBId) return;
  ...
  const handleAddUrl=async()=>{
    ...
    try { const res=await addUrlSource(activeNBId,srcUrl.trim()); setNotebooks((nbs)=>nbs.map((nb)=> (nb.id===activeNBId?res.data:nb)),
      );
      ...
      } setSrcUrl(""); setShowAddSrcModal(false); showToast("Source added");
      fetchUsage(); }
    catch (err) {
      ...
      } else { showToast("Failed to add source");
      } }
    setSrcLoading(false);
  };
  const handleAddFile=async(e)=>{
    ...
    try { const res=await addFileSource(activeNBId,fd); setNotebooks((nbs)=>nbs.map((nb)=> (nb.id===activeNBId?res.data:nb)),
      );
      // Auto-overview: only present on the notebook's first source. Drop it
      // straight into the chat panel as an AI message, unprompted.
      if (res.data.autoOverview && res.data.autoOverview.trim()) {
      ...
      } setShowAddSrcModal(false); showToast("Source added");
      fetchUsage(); }
    catch (err) {
      const planError = parsePlanError(err);
      if (planError) {
  ...
  const handleStudio=async(item, extra = {}, languageOverride = null)=>{
    ...
    const resolvedLanguage = languageOverride ?? selectedLang; setStudioLoading(item.key);
    try {
      const res=await generateStudioItem(notebookId,
        item.key,
        resolvedLanguage,
        extra);
      const output= {
  ...
  const addNB=async()=>{
    ...
    try { const res=await createNotebook({title:newTitle.trim(),color:newColor,icon:"book"}); setNotebooks((nbs)=>[...nbs,res.data]); setActiveNBId(res.data.id); setView("lm"); setShowNewNBModal(false); setNewTitle(""); setNewColor(NB_COLORS[0]); showToast(`"${res.data.title}" created`);
      ...
      if (planError) {
        ...
        setUpgradeConfig({ featureLabel: planError.message });
      } else { showToast("Failed to create notebook");
      } }
  };
  ...
  const addSect=async()=>{
    ...
    try { const res=await addSection({title:newTitle.trim(),color:newColor,notebookId:activeNBId}); setNotebooks((nbs)=>nbs.map((nb)=> (nb.id===activeNBId?res.data:nb))); const ns=res.data.sections[res.data.sections.length-1]; setActiveSectId(ns.id); setShowNewSectModal(false); setNewTitle(""); setNewColor(SEC_COLORS[0]); showToast("Section created");
      ...
      if (planError) {
        ...
        setUpgradeConfig({ featureLabel: planError.message });
      } else { showToast("Failed to create section");
      } }
  };
  ...
  const addPg=async()=>{
    ...
    try { const res=await addPage({title:newTitle.trim()||null,sectionId:activeSect.id}); setNotebooks((nbs)=>nbs.map((nb)=> (nb.id===activeNBId?res.data:nb))); const us=res.data.sections.find((s)=>s.id===activeSect.id); const np=us?.pages[us.pages.length-1]; if(np) setActivePageId(np.id); setShowNewPageModal(false); setNewTitle(""); showToast("Page added");
      ...
      if (planError) {
        ...
        setUpgradeConfig({ featureLabel: planError.message });
      } else { showToast("Failed to add page");
      } }
  };
  ...
  const doShare= async()=>{ if(!shareEmail.trim() || !activeNBId) return;
    try {
      const res = await shareNotebook(activeNBId, shareEmail.trim()); showToast(res.data?.message ??`Invite sent to ${shareEmail}`); setShareEmail(""); setShowShareModal(false); } catch (err) {
  ...
  const searchRes = searchQ.length>1
    ? notebooks.flatMap(
          (nb)=>nb.sections?.flatMap(
              (s)=>s.pages?.filter(
                    (p)=>p.title?.toLowerCase().includes(searchQ.toLowerCase())||p.content?.toLowerCase().includes(searchQ.toLowerCase())).map((p)=>({nb,section:s,page:p}))??[])??[])
    : [];
    ...
    <div style={{display:"flex",gap:7,flexWrap:"wrap"}}>
      {colors.map((c)=>(
        <button key={c} onClick={()=>onSelect(c)} style={{
          ...
          boxShadow:selected===c?`0 0 0 3px ${t.cardBg}, 0 0 0 5px ${c}`:"none",
  ...
  /* ── LOADING ── */
  if (loading) return (
    <div className={dm} style={cssVars}>
      <div className="snblm-loading">
        <div className="snblm-loading-icon">
          <Loader2 size={24} color="var(--text)" className="snblm-spin"/>
        </div>
        <span style={{fontSize:13,color:"var(--text3)",fontFamily:"var(--font-family)",fontWeight:FONT_WEIGHT.medium}}>
          Loading notebooks…
        </span>
      </div>
    </div>
        ...
        <div className="snblm-grid-header">
          ...
          <div className="snblm-pills">
            <div className="snblm-pill">
              <BookOpen size={12} color="var(--text)" strokeWidth={2.2}/>
              <b>{notebooks.length}</b> Notebooks
            </div>
            <div className="snblm-pill">
              <Folder size={12} color="var(--text)" strokeWidth={2.2}/>
              <b>{totalSects}</b> Sections
            </div>
            <div className="snblm-pill">
              <FileText size={12} color="var(--text)" strokeWidth={2.2}/>
              <b>{totalPgs}</b> Pages
            </div>
              {usage && (
                <div className="snblm-pill">
        ...
        <div className="snblm-grid">
          ...
          <div className="snblm-add-card" onClick={()=>{ setNewTitle(""); setNewColor(NB_COLORS[0]); setShowNewNBModal(true); }}>
            <div style={{width:40,height:40,borderRadius:12,background:"var(--accent-bg2)",border:"1px solid var(--accent-bdr)",display:"flex",alignItems:"center",justifyContent:"center"}}>
              <Plus size={18} color="var(--text)" strokeWidth={2}/>
            </div>
        ...
        <div className="snblm-overlay" onClick={()=>setShowNewNBModal(false)}>
          <div className="snblm-modal" onClick={(e)=>e.stopPropagation()}>
            <div className="snblm-modal-title">
              <div className="snblm-modal-icon"><BookOpen size={15} color="var(--text)" strokeWidth={2.2}/></div>
              New Notebook
            </div>
    ...
    <div className="snblm-root">
      ...
      {renderTopBar(
        <>
          ...
          <div className="snblm-breadcrumb">
            <div style={{width:8,height:8,borderRadius:"50%",background:activeNB?.color??"var(--text)",flexShrink:0,boxShadow:`0 0 0 3px ${activeNB?.color??"var(--text)"}25`}}/>
          ...
          {sources.length===0 ? (
            ...
            <div className="snblm-sources-list snblm-scroll">
              {sources.map((src)=>{
                ...
                const color=SRC_COLORS[src.sourceType]??"var(--text)";
                return (
        ...
        <div className="snblm-chat">
          <div className="snblm-chat-hdr">
            <div className="snblm-chat-hdr-left">
              <MessageSquare size={15} color="var(--text)" strokeWidth={2.2}/>
              Chat with Sources
            </div>
          ...
          <div className="snblm-chat-msgs snblm-scroll">
            {chatHistoryLoading ? (
              ...
              <div className="snblm-chat-welcome">
                <div className="snblm-chat-welcome-icon">
                  <BookOpen size={30} color="var(--text)" strokeWidth={1.8}/>
                </div>
              ...
              <div key={m.id} className={`snblm-msg ${m.role}`}>
                ...
                <div className="snblm-msg-bubble">
                  {m.role==="ai" ? ( <ReactMarkdown remarkPlugins={[remarkGfm]}>{m.content}</ReactMarkdown>
                        ) : ( m.content
                        )}
                      </div>
                      {m.role === "ai" && (
          ...
          <div className="snblm-chat-input-row">
            <div className="snblm-chat-input-wrap">
              <textarea
                  ref={chatInputRef} className="snblm-chat-input" placeholder="Ask anything about your sources…" value={chatInput} onChange={(e)=>setChatInput(e.target.value)} onKeyDown={handleChatKey} rows={1}/>
        ...
        <div className="snblm-studio snblm-scroll">
          <div className="snblm-studio-hdr">
            <div className="snblm-studio-hdr-icon">
              <Sparkles size={13} color="var(--text)" strokeWidth={2}/>
            </div>
            Studio
          </div>
          ...
          <div className="snblm-studio-banner">
            ...
            <div style={{fontSize:10,color:"var(--text3)",marginBottom:6}}>
              Selected: <b style={{color:"var(--text)"}}>{selectedLang}</b>
            </div>
          ...
          <div className="snblm-studio-grid">
            {STUDIO_ITEMS.map((item)=>(
              <button key={item.key} className="snblm-studio-btn" onClick={()=> {
      ...
      {showAddSrcModal && (
        <div className="snblm-overlay" onClick={()=>setShowAddSrcModal(false)}>
          <div className="snblm-modal snblm-modal-wide" onClick={(e)=>e.stopPropagation()}>
            <div className="snblm-modal-title">
              <div className="snblm-modal-icon"><Upload size={15} color="var(--text)" strokeWidth={2.2}/></div>
              Add Source
            </div>
            ...
            {srcTab==="file" && (
              <>
                <div className="snblm-drop-zone" onClick={()=>fileInputRef.current?.click()}>
                  <Upload size={28} color="var(--text)" strokeWidth={1.8}/>
            ...
            {(srcTab==="url"||srcTab==="yt") && (
              <div style={{display:"flex",gap:8}}>
                ...
                <button className="snblm-btn" onClick={handleAddUrl} disabled={srcLoading}>
                  {srcLoading ? ( <Loader2 size={13} className="snblm-spin"/>
                    ) : ( <Plus size={13}/>
                    )}
                </button>
              </div>
      ...
      {/* Customize Infographic Modal */}
        ...
        <div className="snblm-overlay" onClick={()=> setShowInfographicModal(false)}
          >
            <div
            ...
            <div className="snblm-modal-title">
              <div className="snblm-modal-icon">
                {(()=>{ const item=STUDIO_ITEMS.find(
                      (i)=>i.key=== activeStudioOutput.type); return item ? ( <item.icon size={15} color="var(--text)" strokeWidth={2}/>
                    ) : ( <Brain size={15} color="var(--text)" strokeWidth={2}/>
                    ); })()}
              </div>
              {STUDIO_ITEMS.find((i) => i.key === activeStudioOutput.type)
                  ?.label ?? activeStudioOutput.type}
              <span style={{fontSize:11,color:"var(--text3)",fontWeight:500,marginLeft:4}}>· {activeStudioOutput.language}</span>
                </div>

              {renderStudioBody(activeStudioOutput
              )}
            <div className="snblm-modal-actions">
              <button className="snblm-cancel" onClick={()=> setActiveStudioOutput(null)}>Close</button>
              {activeStudioOutput.downloadUrl&&( <a className="snblm-btn-ghost" style={{flex:1,
                      justifyContent: "center",
                      textDecoration: "none",
                      display: "inline-flex",
                      alignItems: "center",
                    ...
                    target="_blank"
                    rel="noreferrer"
                    download><Download size={13}/> Download
                  </a>
              )}
                {(activeStudioOutput.type === "report" ||
                  activeStudioOutput.type === "datatable") && (
              <button className="snblm-btn" style={{flex:2}} onClick={()=>navigator.clipboard.writeText(activeStudioOutput.textContent ?? "").then(()=>showToast("Copied!"))}>
                <Copy size={13}/> Copy
              </button>
                )}
            </div>
      ...
      {showShareModal && (
        <div className="snblm-overlay" onClick={()=>setShowShareModal(false)}>
          <div className="snblm-modal" onClick={(e)=>e.stopPropagation()}>
            <div className="snblm-modal-title">
              <div className="snblm-modal-icon"><Share2 size={15} color="var(--text)" strokeWidth={2.2}/></div>
              Share Notebook
            </div>
    ...
    <div className="snblm-root">
      ...
      {renderTopBar(
        <>
          ...
          <div className="snblm-breadcrumb">
            <div style={{width:8,height:8,borderRadius:"50%",background:activeNB?.color??"var(--text)",flexShrink:0}}/>
        ...
        <div className="snblm-note-panel">
          ...
          <div className="snblm-toolbar">
            ...
            <div style={{position:"relative"}}>
              ...
              {showTagMenu && (
                <div style={{position:"absolute",top:"110%",left:0,zIndex:50,background:"var(--modal)",border:"1px solid var(--border2)",borderRadius:12,padding:6,width:158,boxShadow:"var(--shadow-hov)"}}>
                  {TAG_OPTIONS.map((tag)=>(
                    <button key={tag.label} style={{display:"flex",alignItems:"center",gap:7,width:"100%",border:"none",background:"transparent",padding:"6px 8px",borderRadius:7,cursor:"pointer",fontFamily:"var(--font-family)",fontSize:11,fontWeight:600,color:tag.color,textAlign:"left"}} onClick={()=>inTag(tag)} onMouseEnter={(e)=>
                        (e.currentTarget.style.background=tag.bg)} onMouseLeave={(e)=>
      ...
      {showNewSectModal && (
        <div className="snblm-overlay" onClick={()=>setShowNewSectModal(false)}>
          <div className="snblm-modal" onClick={(e)=>e.stopPropagation()}>
            <div className="snblm-modal-title">
              <div className="snblm-modal-icon"><Folder size={15} color="var(--text)" strokeWidth={2.2}/></div>
              New Section
            </div>
      ...
      {showNewPageModal && (
        <div className="snblm-overlay" onClick={()=>setShowNewPageModal(false)}>
          <div className="snblm-modal" onClick={(e)=>e.stopPropagation()}>
            <div className="snblm-modal-title">
              <div className="snblm-modal-icon"><FileText size={15} color="var(--text)" strokeWidth={2.2}/></div>
              New Page
            </div>
